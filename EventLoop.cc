...
// 接受事件之后，执行handleRead()回调从eventfd调用read，避免被再次唤醒
...
void EventLoop::loop()
{
    ...
    while(!quit_)
    {
         ...
         * mainloop->wakeup subloop唤醒后，执行下面的方法，执行之前mainloop注册的cb操作
         */ 
        doPendingFunctors();
    }
...
void EventLoop::doPendingFunctors() // 执行回调
{
    ...
    {
        std::unique_lock<std::mutex> lock(mutex_);
        functors.swap(pendingFunctors_);    // 相当于把pendingFunctors_置空, 把需要执行的回调放到functors中
    }
